...
export const deleteArticle = async (req, res) => {
  ...
  try {
    let article = await prisma.post.findUnique({
      where: { id: data.id, author: { id: req.user.id } },
    });
...
export const updateArticle = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Post updated successfully", article });
  } catch (err) {
    console.error(err);
    if (err instanceof PrismaClientValidationError) {
      throw new CustomValidationError("Invalid fields");
  ...
  try {
    ...
    await prisma.comment.delete({
      where: {
        id: data.commentId,
        post: { id: data.articleId },
        OR: [{ user: { id: req.user.id } }, { user: { id: article.authorId } }],
      },
    });
  } catch (err) {
    if (err instanceof PrismaClientValidationError) {
